 ...
 * Serves both API and frontend static files in production
 */
...
scanner.on('progress', (progress: ScanProgress) => {
  broadcast({ type: 'scan_progress', data: progress, timestamp: Date.now() });
});

scanner.on('scan_complete', (result: MarketScanResult) => {
  broadcast({ type: 'scan_complete', data: result, timestamp: Date.now() });
});

scanner.on('error', (error: string) => {
  broadcast({ type: 'error', data: { message: error }, timestamp: Date.now() });
});

scanner.on('scan_started', () => {
  broadcast({ type: 'scan_started', data: { timestamp: Date.now() }, timestamp: Date.now() });
});

scanner.on('scan_stopped', () => {
  broadcast({ type: 'scan_stopped', data: { timestamp: Date.now() }, timestamp: Date.now() });
});
...
║   WebSocket available at ws://0.0.0.0:${String(PORT).padEnd(18)}║
╚═══════════════════════════════════════════════════════════╝
  `);
});
